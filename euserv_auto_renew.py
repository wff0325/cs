...
#
# 2. 环境变量:
#    - 在青龙面板的 "环境变量" 中，添加以下变量。脚本会优先读取环境变量。
#    - 支持多账号，多个值之间用空格隔开。
#
#    -----------------------------------------------------------------------------------
#    | 名称                        | 值                                                 | 备注                                         |
...
#
# 3. 创建定时任务:
#    - 在 "定时任务" 中，新建一个任务，命令填写 `task <你的脚本文件名>`，定时规则自定（例如 `0 9 * * *` 表示每天9点执行）。
#
# =======================================================================================
        ...
        if "To finish the login process please solve the following captcha." not in f.text:
            ...
            if CHECK_CAPTCHA_SOLVER_USAGE:
                try:
                    ...
                    log(f"[Captcha Solver] 查询使用量失败: {e}")
...
def telegram():
    ...
    try:
        ...
        if response.status_code != 200:
            log(f"Telegram Bot 推送失败: {response.text}")
        else:
...
def main_handler(event, context):
    ...
    if not all([USERNAME, PASSWORD, TRUECAPTCHA_USERID, TRUECAPTCHA_APIKEY, MAILPARSER_DOWNLOAD_URL_ID]):
        log("[AutoEUServerless] 关键配置缺失，请检查环境变量设置！")
    ...
    if len(user_list) != len(passwd_list) or len(user_list) != len(mailparser_dl_url_id_list):
        log("[AutoEUServerless] 用户、密码和Mailparser ID的数量不匹配，请检查配置!")
        if TG_BOT_TOKEN and TG_USER_ID:
            telegram()
        exit(1)
    ...
    for i in range(len(user_list)):
        ...
        log(f"[AutoEUServerless] 检测到第 {i + 1} 个账号有 {len(SERVERS)} 台 VPS，正在尝试续期")
        for k, v in SERVERS.items():
            if v:
                if not renew(sessid, s, passwd_list[i], k, mailparser_dl_url_id_list[i]):
                    log(f"[AutoEUServerless] ServerID: {k} 续订错误!")
                else:
                    log(f"[AutoEUServerless] ServerID: {k} 已成功续订!")
            else:
                log(f"[AutoEUServerless] ServerID: {k} 无需更新")
        time.sleep(15)
        check(sessid, s)
        time.sleep(5)
